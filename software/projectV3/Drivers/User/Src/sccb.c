...
I2C_HandleTypeDef hi2c1; // 修改为I2C2句柄
/*****************************************************************************************
...
void SCCB_GPIO_Config (void)
{
  ...
  __HAL_RCC_I2C1_CLK_ENABLE(); // 修改为I2C2时钟
	
	GPIO_InitStruct.Pin 			= SCCB_SCL_PIN | SCCB_SDA_PIN;				   // SCL引脚
  ...
  GPIO_InitStruct.Alternate = GPIO_AF4_I2C1; // 复用功能改为AF4_I2C2
	HAL_GPIO_Init(SCCB_SCL_PORT, &GPIO_InitStruct);
}

void OV2640_I2C1_Init(void)
{
  hi2c1.Instance = I2C1;
  hi2c1.Init.Timing = 0x00707CBB; // 标准模式100kHz（需根据实际系统时钟调整）
...
HAL_StatusTypeDef OV2640_ReadReg(uint8_t reg, uint8_t *data, uint8_t len)
{
  return HAL_I2C_Mem_Read(&hi2c1, OV2640_DEVICE_ADDRESS, reg, I2C_MEMADD_SIZE_8BIT, data, len, 100);
}

// 写入FT6336寄存器（修改为hi2c2）
HAL_StatusTypeDef OV2640_WriteReg(uint8_t reg, uint8_t *data, uint8_t len)
{
  return HAL_I2C_Mem_Write(&hi2c1, OV2640_DEVICE_ADDRESS, reg, I2C_MEMADD_SIZE_8BIT, data, len, 100);
}
...
uint8_t SCCB_WriteReg (uint8_t addr,uint8_t value)
{
	...
	// status = SUCCESS;	// 写入成功
	return  OV2640_WriteReg(addr, &status, 1);
}
/*************************************************************************************************************************************
*	函 数 名:	SCCB_ReadReg
*
