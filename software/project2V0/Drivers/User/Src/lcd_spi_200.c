...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
  ...
  if (hspi->Instance == SPI4)
  {
    ...
    GPIO_LDC_CS_CLK_ENABLE;        // 使能 片选         引脚时钟
    ...
    *******************************************************/
    ...
    // 初始化 SCK、MOSI引脚
    GPIO_InitStruct.Pin = GPIO_PIN_12 | GPIO_PIN_14;   // SCK、MOSI以及片选引脚
    ...
    // 初始化片选引脚，使用软件 SPI 片选
    GPIO_InitStruct.Pin = GPIO_PIN_9;            // SCK、MOSI以及片选引脚
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;  // 复用推挽输出
    GPIO_InitStruct.Pull = GPIO_NOPULL;          // 无上下拉
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW; // 最高速度等级
    HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);
    // HAL_GPIO_WritePin(GPIOE, GPIO_PIN_9, GPIO_PIN_RESET); // 输出片选信号为低电平，始终选择
    ...
    // 初始化 数据指令选择 引脚
    GPIO_InitStruct.Pin = GPIO_PIN_10;             // 数据指令选择 引脚
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;   // 推挽输出模式
    GPIO_InitStruct.Pull = GPIO_NOPULL;           // 无上下拉
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;  // 速度等级低
    HAL_GPIO_Init(LCD_DC_PORT, &GPIO_InitStruct); // 初始化
...
void MX_SPI4_Init(void)
{
  ...
  LCD_SPI.Init.NSSPMode = SPI_NSS_PULSE_DISABLE;            //	使用片选脉冲模式
...
void LCD_DrawCircle2(uint16_t x, uint16_t y, uint16_t r, uint32_t color)
{
  int Xadd = -r, Yadd = 0, err = 2 - 2 * r, e2;
  do
  {
...
void LCD_FillCircle2(uint16_t x, uint16_t y, uint16_t r, uint32_t color)
{
  int32_t D;     /* Decision Variable */
  uint32_t CurX; /* Current X Value */
  uint32_t CurY; /* Current Y Value */
