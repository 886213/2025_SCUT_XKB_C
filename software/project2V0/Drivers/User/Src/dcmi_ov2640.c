...
void HAL_DCMI_MspInit(DCMI_HandleTypeDef *hdcmi)
{
  ...
  if (hdcmi->Instance == DCMI)
  {
    ...
    ******************************************************************************/
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_1 | GPIO_PIN_0 | GPIO_PIN_5 | GPIO_PIN_4 | GPIO_PIN_6;
...
void OV2640_DMA_Init(void)
{
  ...
  HAL_NVIC_SetPriority(DMA2_Stream7_IRQn, 0, 10);     // 设置中断优先级
...
int8_t DCMI_OV2640_Init(void)
{
  ...
  if ((Device_ID == 0x2640) || (Device_ID == 0x2642)) // 进行匹配，实际的器件ID可能是 0x2640 或者 0x2642
  {
    ...
    // OV2640_Set_Pixformat(Pixformat_JPEG); // 设置OV2640输出的图像格式

    OV2640_Set_Brightness(-2);
    OV2640_Auto_Exposure(0);
    OV2640_Light_Mode(1);
    // OV2640_Set_Saturation(-2);
    // OV2640_Set_Contrast(-2);
   OV2640_Set_Effect(OV2640_Effect_BW);
...
void OV2640_Light_Mode(u8 mode)
{
	...
	{ 
		case 0://auto 
			...
			SCCB_WriteReg(0XC7,0X00);//AWB ON 
			return;  	
		case 2://cloudy
			regccval=0X65;
			regcdval=0X41;
	...
	SCCB_WriteReg(0XFF,0X00);	 
	SCCB_WriteReg(0XC7,0X40);	//AWB OFF 
	SCCB_WriteReg(0XCC,regccval); 
	SCCB_WriteReg(0XCD,regcdval); 
	SCCB_WriteReg(0XCE,regceval);  
}
